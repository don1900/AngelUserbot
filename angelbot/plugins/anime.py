...
async def _(event):
    ...
    try:
        ...
        await angel.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)
    

@bot.on(angel_cmd(pattern="character ?(.*)"))
...
async def _(event):
    ...
    try:
        ...
        await angel.delete(cap_text)
    if os.path.exists(img):
        os.remove(img)


@bot.on(angel_cmd(pattern="fillers ?(.*)"))
...
async def canon(event):
    ...
    nub = await eor(event, f"Searching Filler Episodes For `{angel}`")
    hel_ = search_filler(angel)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{angel}`")
    ...
    if len(list_) == 1:
        ...
        return
    angelbot = f"**📃 Filler Episode Lists :** \n\n"
    for i in list_:
        ...
        angelbot += f"• [{i}]({paste})\n"
    await nub.edit(angelbot)


@bot.on(angel_cmd(pattern="airing ?(.*)"))
@bot.on(sudo_cmd(pattern="airing ?(.*)", allow_sudo=True))
async def _(event):
    query = event.text[8:]
    angel = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(angel, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
