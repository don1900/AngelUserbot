...
@bot.on(angel_cmd(pattern=r"inviteall ?(.*)"))
@bot.on(sudo_cmd(pattern=r"inviteall ?(.*)", allow_sudo=True))
async def get_users(event):
    hel_ = event.text[11:]
    angel_chat = hel_.lower()
    restricted = ["@angelbot_chat", "@its_angelbot"]
    angel = await eor(event, f"__Inviting members from__ {hel_}")
    if angel_chat in restricted:
        await angel.edit("You can't Invite Members from there.")
    ...
    f = 0
    error = "None"
    await angel.edit("**INVITING USERS !!**")
...
@bot.on(angel_cmd(pattern=r"add ?(.*)"))
