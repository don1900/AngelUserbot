...
async def set_not_afk(event):
    ...
    if "#" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        ...
        try:
            await unsave_gif(event, angelbot)
        except:
            pass
        try:
            ...
            await bot.send_message(  # pylint:disable=E0602
                ...
                + f" Ask in {angel_grp} to get help!",
...
@bot.on(angel_cmd(pattern=r"afk ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    owo = event.text[5:]
    reason = owo
    angelpic = await event.client.download_media(krakenop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if owo == "":
            USER_AFK = f"yes: not-mentiond {angelpic}"  # pylint:disable=E0602
            x = await bot.send_message(
                event.chat_id, f"**I'm going afk🚶**", file=angelpic)
            try:
            ...
            try:
                xy = await bot.send_message(
                    Config.LOGGER_ID,
                    f"#AFKTRUE \n**AFK mode** = `True`\n**Reason:** `Not Mentioned`",file=angelpic
                    )
                try:
