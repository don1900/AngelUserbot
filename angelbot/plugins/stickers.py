...
from angelbot.sql.gvar_sql import addgvar, gvarstat, delgvar
from . import *
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"angelbot_{un_}_{pack}"
        packnick = (
            f"{angelbot} Vol.{pack}"
            if angelbot
        ...
        if (
            ...
            async with bot.conversation("Stickers") as conv:
                ...
                while "120" in x.text:
                    pack += 1
                    packname = f"angelbot_{un_}_pack_{pack}"
                    packnick = (
                        f"{angelbot} Vol.{pack}"
                        if angelbot
                        else f"{un}'s Hêllẞø† Vol.{pack}"
        ...
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ForGo10God,
                    title=pname,
                    short_name=f"angel_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@AngelFonts")
