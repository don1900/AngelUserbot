...
async def print_changelbotogs(event, ac_br, changelbotog):
    changelbotog_str = (
        f"🔥 **New UPDATE available for [{ac_br}]:\n\n📑 CHangelbotOG:**\n`{changelbotog}`"
    )
    if len(changelbotog_str) > 4096:
        await event.edit("`Changelbotog is too big, view the file to see it.`")
        with open("output.txt", "w+") as file:
            file.write(changelbotog_str)
...
async def upstream(event):
    ...
    ups_rem.fetch(ac_br)
    changelbotog = await gen_chlog(repo, f"HEAD..upstream/{ac_br}")
    if changelbotog == "" and not force_update:
        await event.edit(
    ...
    if conf == "" and not force_update:
        await print_changelbotogs(event, ac_br, changelbotog)
        await event.delete()
...
async def upstream(event):
    ...
    off_repo = "https://github.com/The-angelbot/angelbot"
    os.chdir("/app")
    git_angel = f"rm -rf .git"
    try:
        await runner.runcmd(git_angel)
    except BaseException:
        pass
    try:
        txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
...
CmdHelp("update").add_command(
  ...
  "update build", None, "Hard-Update Your Hêllẞø†. This won't take you back to your previous deploy. This will be triggered even if there is no changelbotog."
).add_info(
  "Hêllẞø† Updater."
).add_warning(
  "✅ Harmless Module."
