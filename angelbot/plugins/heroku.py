...
from angelbot.sql.gvar_sql import addgvar, delgvar, gvarstat
from . import *
...
async def re(angel):
    if angel.fwd_from:
        return
    event = await eor(angel, "Restarting Hêllẞø† ...")
    try:
        await restart(event)
...
async def down(angel):
    if angel.fwd_from:
        return
    event = await eor(angel, "`Turing Off Hêllẞø†...`")
...
async def sett(event):
    ...
    val_ = hel_.split(" ")[1:]
    valu = " ".join(val_)
    angel = await eor(event, f"**Setting variable** `{var_}` **as** `{valu}`")
    if var_ == "":
        return await angel.edit(f"**Invalid Syntax !!** \n\nTry: `{hl}svar VARIABLE_NAME variable_value`")
    elif valu == "":
        return await angel.edit(f"**Invalid Syntax !!** \n\nTry: `{hl}svar VARIABLE_NAME variable_value`")
    if var_ not in config_list:
        return await angel.edit(f"__There isn't any variable named__ `{var_}`. __Check spelling or get full list by__ `{hl}vars`")
    try:
        addgvar(var_, valu)
    except Exception as e:
        return await angel.edit(f"**ERROR !!** \n\n`{e}`")
    await angel.edit(f"**Variable** `{var_}` **successfully added with value** `{valu}`")


@bot.on(angel_cmd(pattern="gvar ?(.*)"))
@bot.on(sudo_cmd(pattern="gvar ?(.*)", allow_sudo=True))
async def gett(event):
    var_ = event.pattern_match.group(1).upper()
    angel = await eor(event, f"**Getting variable** `{var_}`")
    if var_ == "":
        return await angel.edit(f"**Invalid Syntax !!** \n\nTry: `{hl}gvar VARIABLE_NAME`")
    if var_ not in config_list:
        return await angel.edit(f"__There isn't any variable named__ `{var_}`. __Check spelling or get full list by `{hl}vars`")
    try:
        sql_v = gvarstat(var_)
        os_v = os.environ.get(var_)
    except Exception as e:
        return await angel.edit(f"**ERROR !!** \n\n`{e}`")
    await angel.edit(f"**OS VARIABLE:** `{var_}`\n**OS VALUE :** `{os_v}`\n------------------\n**SQL VARIABLE:** `{var_}`\n**SQL VALUE :** `{sql_v}`\n")


@bot.on(angel_cmd(pattern="dvar ?(.*)"))
@bot.on(sudo_cmd(pattern="dvar ?(.*)", allow_sudo=True))
async def dell(event):
    var_ = event.pattern_match.group(1).upper()
    angel = await eor(event, f"**Deleting Variable** `{var_}`")
    if var_ == "":
        return await angel.edit(f"**Invalid Syntax !!** \n\nTry: `{hl}dvar VARIABLE_NAME`")
    if var_ not in config_list:
        return await angel.edit(f"__There isn't any variable named__ `{var_}`. Check spelling or get full list by `{hl}vars`")
    try:
        delgvar(var_)
    #    os.environ.pop(var_)
    except Exception as e:
        return await angel.edit(f"**ERROR !!** \n\n`{e}`")
    await angel.edit(f"**Deleted Variable** `{var_}`")
...
async def _(dyno):
   ...
   # event = await eor(dyno, "Downloading Logs...")
    ...
    await eor(dyno, angel_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {angel_mention}\n\n🚀** Pasted**  ")
