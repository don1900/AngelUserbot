...
@bot.on(angel_cmd(pattern="gpro ?(.*)"))
@bot.on(sudo_cmd(pattern="gpro ?(.*)", allow_sudo=True))
async def _(angelevent):
    i = 0
    sender = await angelevent.get_sender()
    me = await angelevent.client.get_me()
    angel = await eor(angelevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await angelevent.get_chat()
    if angelevent.is_private:
        user = angelevent.chat
        rank = angelevent.pattern_match.group(1)
    else:
        angelevent.chat.title
    try:
        user, rank = await get_full_user(angelevent)
    except:
        pass
    if me == user:
       k = await angel.edit("You can't promote yourself...")
       return
    try:
        if not rank:
            rank = "ㅤ"
    except:
        return await angel.edit("**ERROR !!**")
    if user:
        telchanel = [d.entity.id
                     for d in await angelevent.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await angelevent.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await angel.edit(f"**Promoting User in :**  `{i}` Chats...")
          except:
             pass
    else:
        await angel.edit(f"**Reply to a user !!**")
    await angel.edit(
...
@bot.on(angel_cmd(pattern="gdem ?(.*)"))
@bot.on(sudo_cmd(pattern="gdem ?(.*)", allow_sudo=True))
async def _(angelevent):
    i = 0
    sender = await angelevent.get_sender()
    me = await angelevent.client.get_me()
    angel = await eor(angelevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await angelevent.get_chat()
    if angelevent.is_private:
    ...
    try:
        ...
        pass
    if me == user:
       k = await angel.edit("You can't Demote yourself !!")
       return
    try:
        if not rank:
            rank = "ㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await angelevent.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await angel.edit(f"**Demoting Globally In Chats :** `{i}`")
          except:
             pass
    else:
        await angel.edit(f"**Reply to a user !!**")
...
async def _(event):
    ...
    if a is not None:
        ...
        gbpic = cjb
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **beta majdur ko khodna 😪 aur** {angel_mention} **ko chodna... Kabhi sikhana nhi!! 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {angel_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
    ...
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                ...
                chats += 1
                await angel.edit(f"**Ungban in progress...** \n**Chats :** __{chats}__")
            except BaseException:
                pass
    ungbaner(userid)
    await angel.edit(
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            angel = int(hel)
            try:
                tity = await event.client.get_entity(angel)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={angel}) (`{angel}`)\n"
    else:
    ...
    async for gkick in event.client.iter_dialogs():
        if gkick.is_group or gkick.is_channel:
            try:
                ...
                chats += 1
                await angel.edit(f"**Kicking globally...** \n**Chats :** __{chats}__")
            except BaseException:
                pass
    a = gvarstat("BAN_PIC")
    if a is not None:
