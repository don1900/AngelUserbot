...
@bot.on(angel_cmd(pattern="picgen"))
...
async def _(event):
    ...
    if response.status_code == 200:
      with open("angelbot.jpg", 'wb') as f:
        f.write(response.content)
    
    captin = f"Fake Image By {angel_mention}"
    fole = "angelbot.jpg"
    await bot.send_file(event.chat_id, fole, caption=captin)
    await event.delete()
    os.system("rm /root/angelbot/angelbot.jpg ")


@bot.on(angel_cmd(pattern="fake ?(.*)"))
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1432756163:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [AngelBoy](https://t.me/ForGo10God) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
