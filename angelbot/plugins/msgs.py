...
async def all_groups_id(angel):
    angelgroups = []
    async for dialog in angel.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            angelgroups.append(entity.id)
    return angelgroups


@bot.on(angel_cmd(pattern="frwd$"))
...
@bot.on(angel_cmd(pattern="resend$"))
...
@bot.on(angel_cmd(pattern=r"fpost (.*)"))
