...
async def muth(angel):
    if angel.is_private:
        ...
        if angel.reply_to_msg_id:
            ...
            return await eod(angelevent, "I Need a user to mute!!")
        if userid == ForGo10God:
            return await eod(angelevent, "Nashe me hai kya lawde")
        if str(userid) in DEVLIST:
            return await eod(angelevent, "**Error Muting God**", 7)
        if ForGo10God not in admin_:
            return await eod(angelevent, NO_PERM)
        if userid in admin_:
            if is_muted(userid, angel.chat_id):
                return await angelevent.edit("Admin already muted ♪～(´ε｀ )")
            try:
                mute(userid, angel.chat_id)
            except Exception as e:
                await angelevent.edit(f"**Error :** \n\n`{e}`")
            else:
                return await angelevent.edit(f"**🌝 Muted admin** [{name}](tg://user?id={userid}) **in** `{chat.title}` (~‾▿‾)~")
        try:
...
async def nomuth(evn):
    if evn.is_private:
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(angelevent, "I need a user to unmute!!")
        if ForGo10God not in admin_:
            return await eod(angelevent, NO_PERM)
        if userid in admin_:
            if not is_muted(userid, evn.chat_id):
                return await angelevent.edit("Not even muted.")
            try:
                unmute(userid, evn.chat_id)
            except Exception as e:
                await angelevent.edit(f"**Error :** \n\n`{e}`")
            else:
                return await angelevent.edit("**Successfully Unmuted** [{name}](tg://user?id={userid}) **in** `{chat.title}`")
        try:
