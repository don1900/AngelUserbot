...
async def lg1(angelevent):
    ...
    fnt = await get_font_file(angelevent.client, "@ANGEL_FRONTS")
    if angelevent.reply_to_msg_id:
        rply = await angelevent.get_reply_message()
        logo_ = await rply.download_media()
    else:
        async for i in bot.iter_messages("@angelbot_LOGOS", filter=InputMessagesFilterPhotos):
    	    PICS_STR.append(i)
    ...
    await bot.send_file(
        angelevent.chat_id,
        file_name,
        caption=f"**Made By :** {angel_mention}",
    )
    await event.delete()
    try:
        os.remove(file_name)
