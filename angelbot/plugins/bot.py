...
async def leave(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("😪 **KThnxBye** See u all in angel!!")
        time.sleep(1)
        if "-" in str(e.chat_id):
...
@bot.on(angel_cmd(pattern="vars"))
@bot.on(sudo_cmd(pattern="vars", allow_sudo=True))
async def lst(event):
    angel = await eor(event, "Getting configs list...")
...
CmdHelp("bot").add_command(
    ...
    "schd", "<secs> - <message>", "Sends your message in given secs", "schd 10 - Angelo"
).add_command(
