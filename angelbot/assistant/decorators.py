...
def angelbot_cmd(add_cmd, is_args=False):
    def cmd(func):
        angelbot = bot.tgbot
        if is_args:
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            angelbot = bot.tgbot
            perms = await angelbot.get_permissions(event.chat_id, event.sender_id)
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            angelbot = bot.tgbot
            boat = await angelbot.get_me()
            perms = await angelbot.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
